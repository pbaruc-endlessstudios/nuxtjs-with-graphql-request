...
async function createRegularClient(url) {
  return new GraphQLClient(url, {
    headers: {
      authorization: 'Bearer 2.0973240577628616'
    }
  })
}
...
export default {
  ...
  methods: {
    doIt: async function() {
      const client = await createRegularClient("http://localhost:3003/graphql");
      const data = await client.request(gql`
            {
              getUserPathwayById(input: { user_pathway_id: 3814 }) {
                user_pathway_nodes {
                    node {
                      id
                      name
